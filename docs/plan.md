...
- [x] Маршрут `/tournaments/new` — форма создания
...
- [x] Маршрут `/tournaments/:id/schedule`
- Двухколоночный вид: левая — неназначенные матчи, правая — назначенные по кортам
- Левая колонка: матчи упорядочены по номеру тура (round_robin) или раунду сетки (playoff); карточки **draggable**
- Правая колонка: табы с датами (DD) фильтруют назначенные матчи; внутри корта — обратный порядок по времени
- Drop-зона на каждом корте (сверху блока): появляется при перетаскивании, если все матчи корта сыграны или матчей нет; время = последний сыгранный матч + 60 мин, по умолчанию 09:00
- Drag & drop: перетащить матч из левой колонки на drop-зону = назначить корт и время без диалога
- Диалог назначения слота (корт + datetime-local) с проверкой конфликтов по игрокам (через `validateConflicts`); при конфликте — предупреждение с возможностью сохранить
- Ввод и редактирование результата матча прямо из колонки Scheduled (тот же диалог партий, что в GroupsView)
- Кнопка «Schedule» добавлена в навигацию TournamentDetail
- `ScheduleService`: `getOrderOfPlay`, `listScheduled`, `listUnscheduled` — список матчей турнира с обогащением (корт, команды, раунд, тур, bracketRound)
- `matches.getById` — новый IPC-метод для загрузки полного `MatchWithTeams` по id
- **Результат:** организатор видит order of play по кортам, назначает матчи мышью или через диалог, вводит результаты не выходя из экрана расписания
